...
use gx_control_plane::message::{ClientRequest, EngineEvent, Envelope};
...
fn handle_connection<P: EnginePeer>(mut stream: TcpStream, peer: Arc<Mutex<P>>) -> io::Result<()> {
    ...
    loop {
        ...
        let (reply, events) = {
            ...
            let reply = engine.handle(envelope.payload);
