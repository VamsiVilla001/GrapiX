...
import { setThumbnailSourceRenderer } from "../rendering/thumbnailCapture";
...
export function GpuSceneStage(props: {
  ...
  useEffect(() => {
    ...
    // The scene viewport is the thumbnail source. Material previews mount their own
    // renderers and must not claim it — a thumbnail of a material sphere is not a
    // thumbnail of the scene.
    setThumbnailSourceRenderer(renderer);
    ...
    return () => {
      cancelled = true;
      setThumbnailSourceRenderer(null);
      renderer.destroy();
