...
type ObjectManagerView = "Objects" | PropertyInspectorTab;
...
export function ObjectManager() {
  ...
  const [view, setView] = useState<ObjectManagerView>("Objects");
  ...
  function selectView(nextView: ObjectManagerView) {
    setView(nextView);
  ...
  function commitLayerRename(layerId: string) {
    if (renameValue.trim() && !renameLayer(layerId, renameValue)) {
      window.alert(`A layer named "${renameValue.trim()}" already exists. Layer names must be unique.`);
    }
      ...
      for (const mask of object.masks ?? []) {
        rows.push(
          ...
          >
            ...
            {transformColumns.map((column) => (
              <div className={`scene-mask-property ${column.id === "alpha" ? "alpha-mask-material-control" : ""}`} key={column.id}>
                {column.id === "alpha" ? (
                  <input
                    aria-label={`Alpha mask opacity for ${object.name}`}
                    max={100}
                    min={0}
