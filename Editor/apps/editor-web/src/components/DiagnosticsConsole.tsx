 ...
 * The author console.
 *
 * The editor used to report a failure as one line in a window.alert or a save indicator that
 * the next action replaced. So `FONT_RESOLUTION_FAILED` was the whole of what an author could
 * ever know: a code with no family, no URL, no remedy, and no way to look at it again once it
 * was gone. On the packaged desktop build there is no devtools console to fall back on.
 *
 * This is that missing surface — the same console Playout's operators have. It shows every
 * failure the project service refused a request with, and everything this window itself could
 * not do — each with the cause, the remedy, and the identifiers needed to act. Records
 * persist until cleared, so a failure during a publish can be read after the publish.
 *
 * Deliberately a drawer rather than a dock panel: it is opened when something is wrong and
 * must not take space from Object Manager, Inspector or the viewport when nothing is.
 * Ported from Playout's DiagnosticsConsole and adapted to the editor's shell.
 */
...
import {
  clearDiagnosticsOnApi,
  listDiagnosticsFromApi,
...
export function DiagnosticsConsole({ open, onClose }: DiagnosticsConsoleProps) {
   ...
   *
   * A restarted project service starts its log again from 1, which would look like "nothing
  ...
  const loadServiceRecords = useCallback(async () => {
    try {
      const page = await listDiagnosticsFromApi(heldSequence.current);
  ...
  useEffect(() => {
    ...
    return subscribeToDiagnostics(() => void loadServiceRecords());
  }, [open, loadServiceRecords]);

  const records = useMemo(() => {
    const all = [...serviceRecords, ...uiRecords];
    // Newest first: during a failure the newest record is the one being read.
    all.sort((left, right) =>
      right.at === left.at ? right.sequence - left.sequence : right.at.localeCompare(left.at)
    );
  ...
  const clearAll = useCallback(async () => {
    ...
    setServiceRecords([]);
    try {
      const { latestSequence } = await clearDiagnosticsOnApi();
      ...
      recordUiDiagnostic({
        level: "warning",
        source: "ui/console",
        message: "Could not clear the service-side log",
        error
      });
    ...
    <section className="diagnostics-console" aria-label="Diagnostics console">
      <header>
        ...
        <button
          ...
          title="Clear every record, here and in the project service"
        >
          <Trash2 size={13} />
          Clear
        </button>
      ...
      {fetchFailure && (
        <p className="console-note">
          Showing this window's records only — the project service's log could not be read: {fetchFailure}
        </p>
      )}
                ...
                <div className="console-detail">
                  ...
                  <div className="console-detail-actions">
                    <span className="console-origin">
                      {record.origin === "ui" ? "This editor window" : "Project service"} · {record.at}
                    </span>
        ...
        {records.length === 0 && (
          <p className="console-empty">
            {query || level !== "all"
              ...
              : "Nothing has failed since this log was cleared. Failures, refused requests and anything this window cannot do are recorded here with their cause and remedy."}
          </p>
        )}
      </div>
    </section>
 ...
 *
 * Everything is included, in reading order. An author copying a failure has no way to know
 * which field an engineer needs.
 */
function asPlainText(record: ConsoleRecord): string {
  const lines = [
    `[${record.at}] ${record.level.toUpperCase()} ${record.source} (${record.origin === "ui" ? "editor window" : "project service"})`,
    record.message
  ];
  ...
  if (record.detail?.context) {
    for (const [name, value] of Object.entries(record.detail.context)) {
      lines.push(`${name}: ${renderValue(value)}`);
    }
  }
