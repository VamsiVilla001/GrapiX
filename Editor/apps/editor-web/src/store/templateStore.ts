...
export const useTemplateStore = create<TemplateState>((set, get) => ({
  ...
  addNewTemplate: () => {
    ...
    // Born at the project resolution, so it never needs conforming afterwards.
    const project = useProjectStore.getState().settings;
    const template = sceneToTemplateScene(
      createEmptyTemplateScene(nextId, project),
      nextId - 1
    );
