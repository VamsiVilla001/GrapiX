...
| 12 | Renderer design | Native Rust/wgpu renderer, shared shader rules, adapter seam, explicit unsupported-feature errors. | Partial | Headless wgpu renders rects/ellipses, Unicode text, vector-shape fill/stroke paths, and depth-tested textured primitive/imported glTF meshes with shared WGSL and pixel smoke tests. General 2D images, paint/line objects, video, effects, and native shape masks remain explicit gaps. |
